...
// ever calls these two functions; each platform provides its own
// .cpp implementing them:
//
...
//
// Note the types: the contract speaks C++ (bool), not C (gboolean).
// Conversions to C APIs happen inside each platform .cpp, at the
// boundary — never in app code.
// ─────────────────────────────────────────────────────────────
#pragma once
